...
* 
 */
public class PriceEditText extends EditText implements
...
//				if(getSelectionStart() != getSelectionEnd()) return;
			...
			public void afterTextChanged(Editable s) {
				...
				if (isChanged) {
					...
					int konggeNumberC = 0;
					int p = -1;
					while (index < buffer.length() && weightNum != 0){
						if(index % (weightNum+1) == weightNum){
							if(buffer.toString().contains(".")){
								p = buffer.toString().indexOf(".");
								if(index < p){
									buffer.insert(index, strformat);
									konggeNumberC++;
								}
							}else{
								buffer.insert(index, strformat);
